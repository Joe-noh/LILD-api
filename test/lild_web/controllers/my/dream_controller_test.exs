defmodule LILDWeb.My.DreamControllerTest do
  ...
  describe "index" do
    ...
    test "ログインユーザの夢を下書き以外返す", %{conn: conn, owner: owner} do
      dreams = Dreams.dreams_query(owner) |> LILD.Repo.all()

      get(conn, Routes.my_dream_path(conn, :index))
      |> json_response(200)
      |> Map.get("dreams")
      |> Enum.each(fn dream ->
    ...
    test "ログインしていないときは401", %{conn: conn} do
      ...
      |> get(Routes.my_dream_path(conn, :index))
      |> json_response(401)
    end
  end
